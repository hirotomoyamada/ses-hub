...
export const Main: React.FC<PropType> = ({
  ...
  return (
    <div className={`${root.setting_inner} ${styles.main}`}>
      ...
      <Provider user={user} setCreate={setCreate} handleProvider={handleProvider} />

      <Provider social user={user} setCreate={setCreate} handleProvider={handleProvider} />
      ...
      <div className={`${styles.main_col} ${styles.main_col_center}`}>
        ...
        {user?.type !== 'child' && (
          <button type='button' onClick={() => setRemove(true)} className={styles.main_btn_delete}>
            アカウント削除
          </button>
        )}
      </div>
