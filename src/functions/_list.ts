...
import { fetchPosts, homePosts, userPosts, extractPosts } from 'features/post/actions';
...
export const createObserver = (
  ...
  const listHeight = Number(JSON.stringify(list?.current?.getBoundingClientRect().height));
...
export const fetchScroll = async (
  ...
  sort: Sort | undefined,
  type: 'likes' | 'outputs' | 'entries' | 'history' | undefined,
  ...
  if ((side || select) && (index === 'matters' || index === 'resources' || index === 'companys')) {
    await dispatch(
      userPosts({
