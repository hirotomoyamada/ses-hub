  ...
  return (
    <div className={root.main_col}>
      ...
      <div className={`${styles.item} ${styles.item_select}`}>
        <select
          className={`${styles.item_input} ${errors.location?.area && styles.item_input_error}`}
        ...
        {errors?.location?.area?.message && (
          <span className={styles.item_error}>{errors.location.area.message}</span>
        )}
      </div>
    </div>
  );
...
export const PlaceLocation: React.FC = () => {
  const {
    register,
    formState: { errors },
  } = useFormContext<Data['matter']>();
  ...
  return (
    <div className={root.main_col}>
      ...
      <div className={styles.item}>
        <input
          placeholder='道玄坂'
          className={`${styles.item_input} ${errors.location?.place && styles.item_input_error}`}
        ...
        {errors?.location?.place?.message && (
          <span className={styles.item_error}>{errors.location.place.message}</span>
        )}
      </div>
    </div>
  );
