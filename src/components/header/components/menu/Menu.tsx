...
export const Menu: React.FC<PropType> = ({ index, uid, user, handleIndex, page, outputs }) => {
  return !outputs?.length ? (
    <div
      ...
      <button
        ...
        className={`${styles.menu_btn} ${index === 'matters' && styles.menu_btn_active}`}>
        案件
      </button>

      <button
        onClick={() => handleIndex('resources')}
        className={`${styles.menu_btn} ${index === 'resources' && styles.menu_btn_active}`}>
        人材
      </button>
      ...
      {(page === 'search' || page === 'likes' || page === 'entries') &&
        ...
        user?.payment?.option?.freelanceDirect && (
          <button
            ...
            className={`${styles.menu_btn} ${index === 'persons' && styles.menu_btn_active}`}>
            フリーランス
          </button>
        )}
    ...
    <div className={`${styles.menu} ${styles.menu_outputs}`}>
      <span className={styles.menu_outputs_txt}>{outputs.length}件&nbsp;選択中</span>
    </div>
  );
};
