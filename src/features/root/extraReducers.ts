import { ActionReducerMapBuilder, PayloadAction, ArgAction, ErrorAction } from '@reduxjs/toolkit';
...
export const extraReducers = (builder: ActionReducerMapBuilder<State>): void => {
  ...
  builder.addMatcher(
    ...
    (
      ...
      if (action.meta.arg.hasPosts) return;
      ...
      if (action.type.includes('/fetchUser/') && action.meta.arg.index)
        state.index = action.meta.arg.index !== 'companys' ? action.meta.arg.index : 'matters';
