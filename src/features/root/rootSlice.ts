...
export const rootSlice = createSlice({
  ...
  reducers: {
    handleIndex: (state, action: PayloadAction<'matters' | 'resources' | 'companys' | 'persons'>) =>
      reducers.index(state, action),
    handleSearch: (state, action: PayloadAction<Search | undefined>) =>
      reducers.search(state, action),
    handleSort: (state, action: PayloadAction<Pick<Sort, 'status' | 'display'>>) =>
      reducers.sort(state, action),
    handlePage: (state, action: PayloadAction<string>) => reducers.page(state, action),
    handleModal: (state, action: PayloadAction<Modal | undefined>) => reducers.modal(state, action),
    ...
    handleLoad: (state, action: PayloadAction<Partial<Load> | undefined>) =>
      reducers.load(state, action),
  },
